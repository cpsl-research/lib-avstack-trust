...
def make_objects_idx_list(n_clustered=10, n_objects=20, n_sets=5):
    ...
    idx_set_cluster = [np.random.choice(n_sets, size=n, replace=False) for n in n_per_cluster]
    ...
    for i_set in range(n_sets):
        ...
        this_clust = [i_obj + n_lone for i_obj, idxs in enumerate(idx_set_cluster) for idx in idxs if idx == i_set]
...
def test_make_objects_list_1():
    objects_idx_list, idx_set_lone, idx_set_cluster = \
        make_objects_idx_list(n_clustered=0, n_objects=10, n_sets=5)
...
def test_make_objects_list_2():
    objects_idx_list, idx_set_lone, idx_set_cluster = \
        make_objects_idx_list(n_clustered=10, n_objects=10, n_sets=5)
    assert len({x for l in objects_idx_list for x in l}) == 10
    assert sum([len(lst) for lst in objects_idx_list]) > 10
    assert sum([len(lst) for lst in idx_set_cluster]) > 0
...
def test_sampled_clustering():
    ...
    objects_idx_list, idx_set_lone, idx_set_cluster = \
        make_objects_idx_list(n_clustered=n_clusters, n_objects=n_objects, n_sets=n_sets)
    observations = [[objects[i] for i in obj_list] for obj_list in objects_idx_list]

    # Run clustering algorithm
    clusterer = clustering.SampledAssignmentClustering()
    clusters = clusterer(observations, assign_radius=1)
