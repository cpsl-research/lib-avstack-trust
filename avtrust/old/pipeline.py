...
class TrustBasedFusionPipeline(CustomPipeline):
    def __init__(self, trust_pipeline, fusion_pipeline):
        self.trust_pipeline = PIPELINE.build(trust_pipeline)
        self.fusion_pipeline = PIPELINE.build(fusion_pipeline)
        self._last_fusion = None

    @apply_hooks
    def __call__(self, data: Dict[str, Any], *args: Any, **kwargs: Any) -> Any:
        output = {}
        data["tracks"] = self._last_fusion
        output["trust"] = self.trust_pipeline(data, *args, **kwargs)
        output["fusion"] = self.fusion_pipeline(data, *args, **kwargs)
